...
def backtest_pair(cointegrated_pair, hedge_ratio, strategy, initial_account):
    ...
    history_arg = merged.loc[merged['period_start_unix'] < window_size_in_seconds + merged['period_start_unix'][0]]
    strategy.initialise_historical_data(history_p1=history_arg['p1_token1_price_per_token0'], history_p2=history_arg['p2_token1_price_per_token0'])
    ...
    for i in history_remaining.index:
        ...
        signal = strategy.generate_signal(ctx, prices)
    ...
    # plt.plot(history_remaining['period_start_unix'], strategy.upper_thresholds[:-1], label='Upper Threshold')
    # plt.plot(history_remaining['period_start_unix'], strategy.lower_thresholds[:-1], label='Lower Threshold')
    # plt.legend()
    # plt.show()
...
mean_reversion_strategy = Mean_Reversion_Strategy(cointegrated_pair=cointegrated_pair,
                                                    ...
                                                    window_size=NUMBER_OF_DAYS_OF_HISTORY_IN_SECONDS)
